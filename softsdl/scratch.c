...
{
    vecf3 point = {0.0f};
    matf4x4 local_to_world;
    matf4x4_identity(&local_to_world);
    matf4x4_translate(&local_to_world, 2.0f, 3.0f, -3.0f);

    matf4x4_print(&local_to_world);
    putchar('\n');

    vecf3 new_point = transformPoint(&point, &local_to_world);
    vecf3_print(new_point);
    return 0;
}
